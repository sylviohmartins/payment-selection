...
class PaymentSelectionAPI {
    ...
    async checkConnection() {
        try {
            const response = await fetch(`/actuator/health/ping`);
    ...
    async searchPayments(filters, page = 0, size = 10, sort = 'id', direction = 'asc') {
        ...
        try {
            ...
            const response = await fetch(`/api/payments/search?${params}`, {
                method: 'GET',
                headers: {
    ...
    async updateSelection(selectionId, updateData) {
        ...
        try {
            const response = await fetch(`/selections/${selectionId}`, {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async applySelection(selectionId, action = 'PAY', selectedIds = []) {
        ...
        try {
            const response = await fetch(`/selections/${selectionId}/apply`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
...
class MockPaymentAPI {
    ...
    async applySelection(selectionId, action = 'PAY', selectedIds = []) {
        ...
        console.log(`Itens afetados: ${selection.selectedCount}. IDs: ${selectedIds.join(", ")}`);
