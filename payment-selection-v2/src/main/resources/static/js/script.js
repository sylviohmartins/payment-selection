...
async function updateSelectionOnServer() {
    ...
    try {
        ...
        // Esta função agora apenas atualiza o estado local e a UI, se necessário, mas não faz chamadas à API diretamente.
        // As chamadas à API para atualização de seleção são feitas diretamente em selectAllItems, deselectAllItems e applyPayments.
    } catch (error) {
...
function toggleItemSelection(paymentId) {
    ...
    // A API não é chamada para seleção individual. O estado é gerenciado localmente.
    // A API será chamada apenas para ações em massa (Marcar/Desmarcar Todos) ou ao Continuar.
}
...
function toggleSelectAllPage() {
    ...
    // Não chamar API para seleção por página, apenas atualizar estado local
    // A API será chamada apenas para ações em massa (Marcar/Desmarcar Todos) ou ao Continuar.
}
...
async function applyPayments() {
    try {
        if (appState.currentSelection) {
            const currentApi = appState.useRealAPI ? api : mockApi;
            const selectedIds = Array.from(appState.selectedItems);
            await currentApi.applySelection(appState.currentSelection.selectionId, 'PAY', selectedIds);
